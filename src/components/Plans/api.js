...
// CREATE
export const createPlan = async (idUser, obj) => {
  const colRef = collection(db, collectionName, idUser, 'plans');
...
export const updateItem = async (userId, id, obj) => {
  const docRef = doc(db, 'loveApp', userId, 'plans', id);
  await updateDoc(docRef, obj)
}

// READ
export const getPlans = async (idUser) => {
  const colRef = collection(db, collectionName, idUser, 'plans');
